...
- Item 2"""
        ...
        expected_blocks = [
            {
                "type": "heading-one",
                "content": [{"type": "text", "content": "Test Post"}],
            },
            {
                "type": "paragraph",
                "content": [
            ...
            {
                "type": "heading-two",
                "content": [{"type": "text", "content": "Subsection"}],
            },
            {
                "type": "bulleted-list",
                "content": [
        ...
        self.mock_client.post_draft = Mock(
            return_value={"id": "post-123", "title": "Test Post"}
        )
    ...
    async def test_create_draft_from_html(self):
        ...
        result = await self.handler.create_draft(
            title="Test Post", content=html_content, content_type="html"
        )
    ...
    async def test_create_draft_plain_text(self):
        ...
        result = await self.handler.create_draft(
            title="Test Post", content=plain_content, content_type="plain"
        )
    ...
    async def test_update_draft(self):
        ...
        self.mock_client.put_draft = Mock(
            return_value={"id": post_id, "title": "Updated Post"}
        )
    ...
    async def test_publish_draft_immediately(self):
        ...
        self.mock_client.publish_draft = Mock(
            return_value={"id": post_id, "published": True}
        )
...
This is for paid subscribers only."""
        ...
        await self.handler.create_draft(
            title="Test with Paywall", content=content, content_type="markdown"
        )
    ...
    async def test_error_handling(self):
        ...
        with pytest.raises(Exception, match="API Error"):
            await self.handler.create_draft(
                title="Test", content="Content", content_type="plain"
            )
